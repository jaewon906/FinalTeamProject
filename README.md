...
##### 5) 주소 API를 사용하여 주소를 직접 입력하는 것 보다 검색해서 입력할 수 있도록 한다. △ 디자인 수정 필요
...
##### 3) 회원정보 수정은 앞서서 비밀번호 인증을 먼저 수행하고 진행한다. x
...
##### 5) 만약 access토큰(또는 refresh토큰)이 만료되었을때 인증된 refresh토큰(또는 access토큰)을 가지고 있다면 access토큰(또는 refresh 토큰)을 재발급한다. △(재발급은 가능. 재발급과 재인증 한 번에 수행되야함) 
...
##### -> 원인은 setInterval수행 시의 변수값이 그대로임 해당 변수의 값을 변경하는 로직을 짜도 setInterval 내부는 변하지않음 
##### ※ 회원 수정 페이지에서 아이디, 이메일, 생년월일을 제외한 정보들을 수정할 수 있게 했다
##### -> input value에 사용자 정보값을 불러오는데 문제는 value에 담겨서 그런지 input에 값을 입력할 수 없는 현상 발생