...
## 맡은 부분
### 회원 관리 파트 & 관리자 파트 (회원, 상품, 주문 관리) & 결제 파트
...
###  
 ...
 3) 테스트 결제모듈은 PortOne사 에서 제공 받았으며 개발자 모드로 진행하기 때문에 결제는 되나 익일 자정에 환불된다.
...
2) 관리자는 주문 상태를 변경할 수 있다. (주문 완료, 배송 준비중, 배송 중, 배송 완료, 환불 및 교환)
 ...
 3) 회원 관리 페이지와 동일하게 페이징 기능과 정렬기능이 가능하다.
...
1) 접근 시 초기 화면에 최근 주문건 100개를 보여준다
 ...
 2) 관리자는 상품 재고 상태를 변경할 수 있다. (재고 있음, 품절)


 3) 회원, 주문 관리 페이지와 동일하게 페이징 기능과 정렬기능이 가능하다.


 4) 상품 등록 기능은 DB에 등록되지 않은 isbn코드를 입력하면 등록될 수 있게 버튼이 활성화 되고 DB에 등록되어 있으면 해당하는 책 정보를 띄워준다.
...
#### orderDetail/{ordernumber} : 본인 주문 상세 정보 열람 기능
